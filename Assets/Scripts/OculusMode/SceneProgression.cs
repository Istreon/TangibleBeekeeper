...
public class SceneProgression : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(!sceneAudio.isPlaying && canContinue)
        {
            ...
            else if(sceneIndex == 4)
            {
                ...
                foreach (HandInteractor hand in hands)
                {
                    ...
                    hand.onSelectExited.AddListener(PutVisorOn);
                }
        ...
        //Condition for the las step of the tutorial (i.e. putting the visor)
        float deltaT = Time.realtimeSinceStartup - t;
        if((!sceneAudio.isPlaying && canContinue && sceneIndex >= 5) | (sceneIndex >= 5 && deltaT > 30))
        {
            //Debug.Log("LOAD NEXT SCENE");   
            bgm.Stop();
            loadingScreen.gameObject.SetActive(true);
            loadingScreen.StartLoading("HiveScene");  
        }
    }
    ...
    protected virtual void PutVisorOn(XRBaseInteractable interactable)
    {
        if(isPuttingVisor)
        {
