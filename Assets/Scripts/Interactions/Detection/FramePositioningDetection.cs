...
public class FramePositioningDetection : MonoBehaviour
{
    ...
    private void isStillTidy()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (frameDetectionVolume[i] != null)
            {
                Vector3 nearestPointOfHoneycomb = frameDetectionVolume[i].GetComponent<Collider>().ClosestPointOnBounds(gameObject.transform.position);
        ...
        else if (!tidy && res)
        {
            ...
            } else
            {
                if(!tidyButIncorrect)
                {
                    Debug.Log("I am " + id + " and I am in the hive but I am not straight, my positioning is incorrect");
